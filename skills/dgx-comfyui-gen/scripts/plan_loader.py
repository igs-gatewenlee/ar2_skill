...
def _import_sibling_module(module_name: str, file_name: str):
    ...
    # dgx-comfyui-gen/scripts/ → dgx-comfyui-gen/ → skills/ (plugin skills root).
    # Sibling skills are co-located in the plugin (cache or source tree), so a
    # single relative candidate suffices — no absolute ~/Code fallback needed.
    skills_dir = Path(__file__).resolve().parent.parent.parent
    candidates = [
        skills_dir / "dgx-comfyui-plan" / "scripts" / file_name,
    ]
