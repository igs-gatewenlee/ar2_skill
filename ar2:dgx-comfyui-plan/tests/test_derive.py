...
# BC-G9-4 (#009): single base Plan/Item; all fixtures via replace so v1.3 /
# future dataclass fields auto-propagate instead of being silently dropped.
_BASE_PLAN = ps.Plan(
...
_BASE_ITEM = ps.Item(slug="x", prompt="<derived>")


def _mk(slug: str, prompt: str = "<derived>", **ov) -> ps.Item:
...
def _make_plan(
    ...
    return replace(_BASE_PLAN, layer_a=layer_a, layer_b=layer_b)
...
class TestBC7BasicDerive(unittest.TestCase):
    ...
    def test_derive_with_only_locked(self):
        ...
        item = _mk(slug="card_01", prompt="<derived>")
...
class TestBC9Resolution(unittest.TestCase):
    ...
    def test_locked_returns_literal(self):
        ...
        result = pd.derive_prompt(plan, _mk("x_01_y", "<derived>"))
    ...
    def test_per_group_lookup_success(self):
        ...
        result = pd.derive_prompt(plan, _mk("ch1_01_morning", "<derived>"))
    ...
    def test_dr7_per_group_no_layer_b(self):
        ...
        result = pd.derive_prompt(plan, _mk("ch1_01_x", "<derived>"))
    ...
    def test_dr7_per_group_group_missing_from_progression(self):
        ...
        result = pd.derive_prompt(plan, _mk("ch9_01_x", "<derived>"))
        self.assertIn("brown braids", result)
...
class TestBC10Unspecified(unittest.TestCase):
    ...
    def test_unspecified_skipped(self):
        ...
        result = pd.derive_prompt(plan, _mk("card_01", "<derived>"))
...
class TestEH4MissingLayerA(unittest.TestCase):
    ...
    def test_eh4_layer_a_none(self):
        ...
        with self.assertRaisesRegex(ValueError, "EH-4"):
            pd.derive_prompt(plan, _mk("card_01", "<derived>"))
...
class TestEH6EmptyResult(unittest.TestCase):
    ...
    def test_eh6_layer_a_all_per_group_no_layer_b(self):
        ...
        with self.assertRaisesRegex(ValueError, "EH-6"):
            pd.derive_prompt(plan, _mk("ch1_01_x", "<derived>"))
